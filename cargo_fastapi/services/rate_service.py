...
from services.adapters.base import RateRequest, PackageData
...
class RateService:
    ...
    def get_rates(data: dict) -> list:
        ...
        carriers = registry.get_all()
        if carrier_codes:
            carriers = [c for c in carriers if c.__class__.__name__.upper().replace("ADAPTER", "") in [cc.upper() for cc in carrier_codes]]
        ...
        for adapter in carriers:
            try:
                ...
                errors.append({"carrierCode": adapter.get_adapter_name(), "message": str(e)})
